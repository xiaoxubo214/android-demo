...
//                    matrix.postTranslate(event.getX() - pA.x, event.getY() - pA.y);
                    float height = mFocusRect.bottom - mFocusRect.top;
                    mFocusRect.top = event.getY() - height / 2;
                    mFocusRect.bottom = event.getY() + height / 2;
//                    fixTranslation();
//                    setImageMatrix(matrix);
                } else if (mode == ZOOM) {
...
//                    bitmap.recycle();
    ...
    private void saveOutput(Bitmap croppedImage, Bitmap.CompressFormat outputFormat, File saveFile) {
        ...
        try {
            outputStream = getContext().getContentResolver().openOutputStream(FileUtils.file2Uri(getContext(), saveFile));
