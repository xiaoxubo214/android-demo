...
public class UnderlinePageIndicator extends View implements PageIndicator {
    ...
    private int scrollToLeft = 0;
    private int scrollToRight;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final float margin = DensityUtil.dip2px(getContext(), mScaleLength);
        final float pageWidth;
        if(count > MAX_TAB_SIZE){
        	pageWidth =  (getWidth() - paddingLeft - getPaddingRight()) / (1f * MAX_TAB_SIZE);
        }else{
        	pageWidth = (getWidth() - paddingLeft - getPaddingRight()) / (1f * count);
        }

        //final float left = paddingLeft + pageWidth * (mCurrentPage + mPositionOffset) + margin;
        //final float right = left + pageWidth - 2 * margin;
        final float top = getPaddingTop();
        final float bottom = getHeight() - getPaddingBottom();

        
        if(scrollToRight == 0){
        	canvas.drawRect(scrollToLeft, top, pageWidth, bottom, mPaint);
        }else{
        	canvas.drawRect(scrollToLeft, top, scrollToRight, bottom, mPaint);
        }
        
    }

    public void moveToRight(int moveLeft,int moveRight){
    	scrollToLeft = moveLeft;
    	scrollToRight = moveRight;
    	invalidate();
    }
